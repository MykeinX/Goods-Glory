...
- `map_board_silhouette.json` gerçek kıyı verisinden **kodla üretilir** (`build_board.py`, Natural Earth 1:50m). Çizilen sanat değil, stilize edilmiş coğrafyadır: her kara kütlesi kendi içinde kapatılır (boğazlar yapısal olarak korunur), iç sular delinir, mutlak km toleransıyla sadeleştirilir ve **oktilineer düzenlenir** — kıyı da yollar gibi H/V/45° koşulardan oluşur. Renderer köşeleri sabit iOS radius'uyla yumuşatır. Ülke sınırı çizilmez; yakınlaştırma yeni coğrafi detay üretmez.
- **Koordinatların tamamı gerçek WGS84'tür.** Silüet, `city_anchors.json` ve `cities.json` aynı uzayı konuşur; registration / fit / nudge katmanı yoktur. Tahta sanatını gerçeğe oturtmaya çalışan eski katman birkaç yüz km hata taşıyordu (Miami 495 km içeride, Jacksonville'de) ve kaldırıldı — `scripts/assets/board_art_reference.png` yalnız **stil** referansıdır.
- `MapProjection` şehirleri, yolları ve silüeti aynı koordinattan taşır.
...
- **Boğazlar yazarlıdır** (`build_board.py` içindeki `STRAITS`). Kaynak veride Cebelitarık 14 km, İstanbul Boğazı 700 m'dir; bu tahtanın kaldırabileceği hiçbir rasterde bir piksel etmez, yani zaten kaynaşmış gelirler. Tahta ölçeğinde bir boğazı gerçeğinden geniş çizmek kartografik bir karardır — ölçek bilinçli olarak yanlıştır, davranış gerçektir: her boğaz en az bir kafes hücresi genişliğinde oyulur, dolayısıyla yolu da bloke eder. Üzerinden gerçek bir karayolu geçiyorsa (köprü) o yol `WATER_CROSSINGS`'te bildirilir; köprü zaten bir su geçişidir.
- Bugünkü ağ kara ağıdır; deniz taşımacılığı geldiğinde maske ters çevrilerek aynı kafes ve aynı arama ile kendi modlu ağını alır. Oyulmuş boğazlar bunu mümkün kılan şeydir: Cebelitarık açık olduğu için Akdeniz kapalı bir delik değil, Atlantik'e bağlı açık denizdir. Demiryolu da aynı kara maskesini ve aynı kafesi kullanır, yalnız kenar kümesi ayrıdır.
